...
try {
  //empty
    ...
    if (preg_match("/^[0-9]+(.[0-9]*)?$/", $_POST['latitude']) == 0 || $_POST['latitude'] > 90 || $_POST['latitude'] < -90){
      array_push($formErrors, 'latitude need to be floating point.'."\\n");
      $formFlag = true;
    }
    if($_POST['latitude'] < -90 || $_POST['latitude'] > 90){
      array_push($formErrors, 'latitude need to fall between -90 and 90 degrees.'."\\n");
      $formFlag = true;
    }
    if (preg_match("/^[0-9]+(.[0-9]*)?$/", $_POST['longitude']) == 0 || $_POST['longitude'] > 180 || $_POST['latitude'] < -180){
      array_push($formErrors, 'longitude need to be floating point.'."\\n");
      $formFlag = true;
    }
    if($_POST['longitude'] < -180 || $_POST['longitude'] > 180){
      array_push($formErrors, 'longitude need to fall between -180 and 180 degrees.'."\\n");
      $formFlag = true;
    }
    if($formFlag){
